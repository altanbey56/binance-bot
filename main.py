...
# --- Binance.TR API Key ve Secret ---
API_KEY    = os.environ.get("BINANCE_API_KEY", "")
API_SECRET = os.environ.get("BINANCE_API_SECRET", "")

# --- Bot parametreleri ---
SYMBOL      = "COS_TRY"
AMOUNT      = float(os.environ.get("AMOUNT", "100"))  # TRY cinsinden alım miktarı
...
def calculate_ut_bot(df, key_value=1, atr_period=10):
    ...
    buy = sell = False
    if len(src) >= 2:
        above = src[-1] > xATR_ts[-1] and src[-2] <= xATR_ts[-2]
        below = xATR_ts[-1] > src[-1] and xATR_ts[-2] <= src[-2]
...
# --- Yeni Binance.TR ile emir gönder ---
def place_order(symbol, side, try_amount):
    ...
    quantity = round(try_amount / price, 6)
    ...
    r = requests.post(f"{BASE_URL}/open/v1/orders", params=payload, headers=headers)
    result = r.json()
...
def home():
    ...
    <table>
    <tr><th>Zaman</th><th>Tip</th><th>Symbol</th><th>Miktar (TRY)</th><th>Fiyat</th><th>Adet</th><th>Sonuç</th></tr>
    {rows}
    </table>
    </body></html>
    """

# --- Bot aktif/durdur endpoint ---
@app.route("/toggle")
def toggle_bot():
    global bot_active
    bot_active = not bot_active
    return jsonify({"status": "OK", "bot_active": bot_active})

# --- Manuel tetikleme endpoint ---
@app.route("/check")
def manual_check():
    status, data = check_and_trade()
